...
async def start_quiz(update, context, filename):
    ...
    order = order[:30]  # Limite a 30 domande

    # SHUFFLE DELLE RISPOSTE UNA SOLA VOLTA QUI
    # Creiamo una lista con le risposte mescolate per ogni domanda, e salviamo indice corretto coerente
    for q in quiz_data:
        ...
        q["_shuffled_answers"] = new_answers
        q["_correct_index"] = new_correct_index
...
async def send_next_question(user_id, context):
    ...
    if img_path:
        ...
        if full_img_path:
            try:
                ...
                await context.bot.send_message(chat_id=user_id, text=f"Errore caricamento immagine: {e}")
    ...
    keyboard = [[InlineKeyboardButton(chr(65+i), callback_data=f"answer:{i}") for i in range(len(answers))]]
    keyboard.append([
...
async def handle_answer(user_id, answer_index, context):
    ...
    # Controllo sicurezza
    if correct_index == -1 or correct_index >= len(answers) or not answers:
